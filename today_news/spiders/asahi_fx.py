...
class AsahiFxSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "朝日新闻"
    allowed_domains = ["asahi.com", "asahicom.jp"]
    custom_settings = {
    ...
    def start_requests(self):
        # 主sitemap URL
        main_sitemap_url = 'https://www.asahi.com/sitemap.xml'
        # 请求主sitemap
    ...
    def parse_detail(self, response):
        ...
        clean_text = response.xpath('//*[@id="main"]/div[@class="nfyQp"]/p')
        txt_list = []
        for p in clean_text.extract():
            _p = remove_tags(self.clean_phrase(p))
            if _p:
                # print([_p])
                txt_list.append(_p)
        # print('\n'.join(txt_list))
