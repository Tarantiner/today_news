...
class BloombergSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "彭博社"
    # allowed_domains = ["bloomberg.com"]
    start_urls = ["https://www.bloomberg.com/sitemaps/news/latest.xml"]

    def parse_detail(self, response):
        itm = response.meta['item']
        try:
            txt_list = []
            s = response.xpath('//script[@id="__NEXT_DATA__"]/text()').extract_first()
            data = json.loads(s)
            itm_list = data['props']['pageProps']['story']['body']['content']
            for _itm in itm_list:
                if _itm.get('type') == 'paragraph':
