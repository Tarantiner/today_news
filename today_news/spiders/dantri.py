...
class DantriSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "越南先锋报"
    allowed_domains = ["dantri.com.vn"]
    start_urls = ["https://dantri.com.vn/google-news-sitemap.xml"]

    def match_invalid_url(self, url):
        if any(('/the-gioi/' in url, '/thoi-su/' in url)):
            return False
        return True

    def parse_detail(self, response):
        d1 = response.xpath('//article[@class="singular-container"]')
        clean_text = d1.xpath('.//div[@class="singular-content"]//p').xpath('string(.)')
        txt_list = []
