...
class EurasiantimesSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "欧亚时报"
    ...
    def parse_detail(self, response):
        ...
        d1 = response.xpath('//div[contains(@class, "td-post-content")]')
        ...
        if response.request.url == self.start_urls[0]:
            ...
            for link in response.xpath('//loc/text()').extract():
                if link and 'post-sitemap' in link:
                    if link == 'https://www.eurasiantimes.com/post-sitemap.xml':
                        x = 0
                        lis.append((link, x))
                    else:
                        try:
