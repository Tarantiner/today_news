...
class JapanTimesFxSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "日本时报"
    allowed_domains = ["www.japantimes.co.jp"]
    start_urls = ["https://www.japantimes.co.jp/latest-news/", "https://www.japantimes.co.jp/latest-news/?pgno=2"]
    ...
    def parse_detail(self, response):
        ...
        clean_text = response.xpath('//div[@id="jtarticle"]/div[contains(@class, "article-body") and contains(@class, "blurred-text")]/p/text()') or response.xpath('//div[@class="article-info"]/div[@class="article-body"]/p/text()')
        txt_list = []
    ...
    def parse(self, response):
        if response.request.url == self.start_urls[0]:
            ...
            for itm in response.xpath('//div[contains(@class, "jt-latest-news") and contains(@class, "paddingDefault")]/div[@class="article"]'):
                url = response.urljoin(itm.xpath('./div[@class="article-details"]/div[@class="article-title"]/a/@href').extract_first(''))
                if not url:
                    continue
