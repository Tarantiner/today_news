...
class NewTalkSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "新头壳"
    # allowed_domains = ["indsr.org"]
    start_urls = [
        'https://newtalk.tw/news/subcategory/1/國際',
        'https://newtalk.tw/news/subcategory/7/中國',
    ]
    ...
    def parse_with_continuation(self, response):
        ...
        for itm in response.xpath('//ul[contains(@class, "category-list")]//li'):
            url = itm.xpath('./a[1]/@href').extract_first('')
            if not url:
                continue
            ...
            title = self.clean_phrase(
                itm.xpath('./a[1]/@title').extract_first(''))
            if not title:
                continue
            pub_time = self.to_utc_string(self.name, itm.xpath('./a[1]//p[@class="date"]/text()').extract_first('').replace('發布 ', '').strip())
            if not pub_time:
                continue
            # 检查过期资讯并过滤
            if self.settings.get('ENABLE_NEWS_TIME_FILTER') and self.check_expire_news(pub_time, self.settings.get(
            ...
            img_list = itm.xpath('./a[1]/div/img')
            if img_list:
