...
class NypostFxSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "纽约邮报"
    allowed_domains = ["nypost.com"]
    start_urls = ["https://nypost.com/world-news/"]

    def parse_detail(self, response):
        itm = response.meta['item']
        
        title = response.xpath('//*[@id="main"]/article//div[@class="single__header"]/header/h1/text()').extract_first('')
        if title:
            itm['title'] = title
        ...
        clean_text = response.xpath('//*[@id="main"]/article//div[contains(@class,"entry-content")]/p/text()')
        txt_list = []
    ...
    def parse(self, response):
        if response.request.url == self.start_urls[0]:
            ...
            for itm in response.xpath('//*[@id="main"]//div[@class="section"]//div[contains(@class,"the-lates")]//div[@class="layout__item"]//div[@class="story__text"]/h3/a'):
