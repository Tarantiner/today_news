...
class PtsSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "公视新闻网"
    ...
    def parse_detail(self, response):
        ...
        d1 = response.xpath('//div[contains(@class, "post-article")]')
    ...
    def parse_with_continuation(self, response):
        ...
        for itm in response.xpath('//ul[contains(@class, "news-list")]/li'):
            url = itm.xpath('.//h2[@title]/a/@href').extract_first('')
            if not url:
                continue
            if self.settings.get('ENABLE_NEWS_URL_FILTER') and self.match_invalid_url(url):
                continue
            title = self.clean_phrase(
                itm.xpath('.//h2[@title]/a/text()').extract_first(''))
            if not title:
                continue
            pub_time = self.to_utc_string(self.name, itm.xpath('.//ul[@create-time]/@create-time').extract_first('') + '+08:00')
            # 检查过期资讯并过滤
            if self.settings.get('ENABLE_NEWS_TIME_FILTER') and self.check_expire_news(pub_time, self.settings.get(
                    'NEWS_EXPIRE_DAYS')):
