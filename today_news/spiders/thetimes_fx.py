...
class ThetimesFxSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "泰晤士报"
    allowed_domains = ["thetimes.com"]
    start_urls = ["https://www.thetimes.com/sitemaps/news"]

    def parse_detail(self, response):
        itm = response.meta['item']

        clean_text = response.xpath('//article//div[@class="Theme-Layer-BodyText--inner"]/p//text() | //article//div[contains(@class, "responsive__ArticleContent")]/p//text()')
        txt_list = []
        ...
        if not itm.get('images'):
            img_list = response.xpath('//meta[@property="twitter:image"] | //meta[@name="twitter:image"]')
            if img_list:
                img_url = response.urljoin(img_list[0].xpath('./@content').extract_first(''))
                img_caption = img_list[0].xpath('//meta[@property="twitter:image:alt"]/@content').extract_first('')
                img_time = ''
                images = [
