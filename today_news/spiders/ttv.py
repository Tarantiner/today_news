...
class TtvSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "台视新闻网"
    # allowed_domains = ["ttv.com.tw"]
    start_urls = ["https://news.ttv.com.tw/sitemap/sitemap.xml"]
    ...
    def parse_detail(self, response):
        ...
        if not itm.get('images'):
            img_list = d1.xpath('//figure[@class="cover img"]//img')
            if img_list:
    ...
    def parse(self, response):
        if response.request.url == self.start_urls[0]:
            ...
            for itm in response.xpath('//url'):
                url = itm.xpath('./loc/text()').extract_first('').replace('/news/detail/', '/news/')
                if not url:
                    continue
                ...
                content = ''
                source = itm.xpath('./news/genres/text()').extract_first('')
