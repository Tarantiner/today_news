...
class VtcnewsSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "VTC新闻"
    allowed_domains = ["vtcnews.vn"]
    start_urls = ["https://vtcnews.vn/sitemap-news.xml"]

    def parse_detail(self, response):
        d1 = response.xpath('//div[@itemprop="articleBody"]')
        clean_text = d1.xpath('.//p[contains(@class, "expEdit")]').xpath('string(.)')
        txt_list = []
