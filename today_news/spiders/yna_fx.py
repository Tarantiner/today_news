...
class YnaFxSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "韩联社"
    allowed_domains = ["yna.co.kr"]
    start_urls = ["https://cn.yna.co.kr/news","https://cn.yna.co.kr/news/2","https://cn.yna.co.kr/news/3"]

    def parse_detail(self, response):
        itm = response.meta['item']
        
        title = response.css('.story-view-zone.article-view-zone header h1[class="tit"]::text').extract_first('')
        if title:
            itm['title'] = title
        ...
        clean_text = response.css('.story-view-zone.article-view-zone .view-body .story-news p::text')
        txt_list = []
        ...
        if not itm.get('keywords'):
            jsonDataRow = response.xpath('//script[@id="pfrtData"]/text()').extract_first('')
            jsonData = json.loads(jsonDataRow)
            itm['keywords'] = jsonData.get('keyword', '')
    ...
    def parse(self, response):
        if response.request.url == self.start_urls[0]:
            ...
            for itm in response.css('.wrap-container .sub-content ul li article .txt-con .tag ~ .tit a'):
