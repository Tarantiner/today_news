...
class YomiuriSpider(scrapy.Spider, SpiderTxtParser, SpiderUtils):
    name = "读卖新闻"
    ...
    def match_invalid_url(self, url):
        if any(('/politics/' in url, '/economy/' in url, '/world/' in url)):
            return False
        return True

    def parse_detail(self, response):
        if response.xpath('//div[@class="c-article-header-member-only"]'):
            print('需要会员', response.url)
            return
        d1 = response.xpath('//div[@class="p-main-contents "]')
        clean_text = d1.xpath('./p[@itemprop="articleBody"]').xpath('string(.)')
        txt_list = []
